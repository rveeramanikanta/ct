...
BFS.prototype.init = function(am, w, h) {
	showEdgeCosts = false;
	BFS.superclass.init.call(this, am, w, h); // TODO: add no edge label flag
												// to this?
...
BFS.prototype.setup = function() {
	...
	this.cmd("CreateLabel", this.nextIndex++, "Visited", VISITED_START_X
			- AUX_ARRAY_WIDTH, AUX_ARRAY_START_Y - AUX_ARRAY_HEIGHT * 1.5, 0);
	this.cmd("CreateLabel", this.nextIndex++, "BFS Queue", QUEUE_START_X,
			QUEUE_START_Y - 30, 0);
...
BFS.prototype.doBFS = function(startVetex) {
	...
	this.cmd("CreateLabel", queueID[tail], vertex, QUEUE_START_X + queueSize
			* QUEUE_SPACING, QUEUE_START_Y);
	...
	while (queueSize > 0) {
		...
		this.cmd("CreateHighlightCircle", this.highlightCircleAL,
				HIGHLIGHT_CIRCLE_COLOR, this.adj_list_x_start
						- this.adj_list_width, this.adj_list_y_start + vertex
		...
		this.cmd("CreateHighlightCircle", this.highlightCircleAM,
				HIGHLIGHT_CIRCLE_COLOR, this.adj_matrix_x_start
						- this.adj_matrix_width, this.adj_matrix_y_start
						+ vertex * this.adj_matrix_height);
		...
		for (var neighbor = 0; neighbor < this.size; neighbor++) {
			if (this.adj_matrix[vertex][neighbor] > 0) {
				...
				if (!this.visited[neighbor]) {
					...
					this.cmd("Disconnect", this.circleID[vertex],
							this.circleID[neighbor]);
					this.cmd("Connect", this.circleID[vertex],
							this.circleID[neighbor], BFS_TREE_COLOR,
							this.curve[vertex][neighbor], 1, "");
					this.queue[tail] = neighbor;
					this.cmd("CreateLabel", queueID[tail], neighbor,
		...
		for (i = 0; i < queueSize; i++) {
			...
			this.cmd("Move", queueID[nextQueueIndex], QUEUE_START_X + i
					* QUEUE_SPACING, QUEUE_START_Y);
		}
